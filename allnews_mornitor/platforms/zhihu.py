# coding=utf-8
"""知乎 — CDP 抓取热榜。"""

from __future__ import annotations

from typing import List

from allnews_mornitor import cdp_browser, store
from allnews_mornitor.models import Post
from allnews_mornitor.platforms import BasePlatform, register
from allnews_mornitor.platforms.extract_utils import GENERIC_CARD_EXTRACT, parse_cards

EXTRACT_JS = (
    GENERIC_CARD_EXTRACT
    + r"""
const out = [];
const seen = new Set();
document.querySelectorAll("a[href*='/question/'], a[href*='/zvideo/'], a[href*='/answer/']").forEach((a) => {
  const href = abs(a.getAttribute('href') || '');
  if (!href || seen.has(href)) return;
  if (!/zhihu\.com/.test(href)) return;
  const root = a.closest(".HotItem, .ContentItem, div") || a;
  const title = (a.innerText || a.getAttribute('aria-label') || '').trim().slice(0, 200);
  if (!title || title.length < 4) return;
  let likes = 0, comments = 0, views = 0;
  const metrics = root.innerText || '';
  const hm = metrics.match(/([\d.]+)\s*万?\s*热度/);
  if (hm) views = num(hm[0]);
  const am = metrics.match(/([\d.]+[万wW]?)\s*赞同/);
  if (am) likes = num(am[1]);
  const cm = metrics.match(/([\d.]+[万wW]?)\s*评论/);
  if (cm) comments = num(cm[1]);
  seen.add(href);
  out.push({title, url: href, author:'', summary:title, likes, comments, collects:0, shares:0, views});
});
return out.slice(0, 50);
"""
)


@register
class ZhihuPlatform(BasePlatform):
    id = "zhihu"
    name = "知乎"

    def fetch(self) -> List[Post]:
        cfg = store.load_config().get("cdp") or {}
        wait_ms = int(cfg.get("wait_ms") or 2500)
        posts: List[Post] = []
        with cdp_browser.cdp_session() as driver:
            for url in self.entry_urls():
                cdp_browser.navigate_dedicated_tab(driver, url)
                cdp_browser.jitter_sleep(wait_ms)
                cdp_browser.scroll_page(driver, rounds=5, step=800, wait_ms=wait_ms)
                raw = cdp_browser.exec_js(driver, EXTRACT_JS)
                posts.extend(parse_cards(raw, self.id))
        seen, uniq = set(), []
        for p in posts:
            if p.post_id in seen:
                continue
            seen.add(p.post_id)
            uniq.append(p)
        return uniq
